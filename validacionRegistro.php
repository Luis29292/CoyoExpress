<?php
  ...
  if ($conexion) //Si la conexión con la base de datos fue exitosa
  {
    if (isset($_POST['identificador']) && isset($_POST['nombre']) && isset($_POST['apePaterno']) && isset($_POST['apeMaterno']) && isset($_POST['contra'])) //Verificamos que estén definidos (que no sean NULL) los datos enviados en el formulario
    {
      ...
      {
        $sql = sprintf("INSERT INTO usuario (id_Usuario, Nombre, Contrasena) VALUES ('%d', '%s', '%s')", //Almacenamos la inserción que queremos realizar en $consulta2, ocupamos sprintf para esto
                              $identificador,
        ...
        header('Location: iniciarSesion.php'); //Redirigimos al usuario a "iniciarSesion.php" para que acceda al sistema
      }
